...
뷰 컨트롤러 소스 파일에 웹킷 뷰를 연결한 `@IBOutlet` 변수를 추가합니다.
...
뷰 컨트롤러에서 해당 뷰 컨트롤러에 대한 `extension` 코드를 작성합니다.
