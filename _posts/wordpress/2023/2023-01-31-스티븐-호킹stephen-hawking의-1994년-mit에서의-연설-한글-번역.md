...
_(보 속도의)_ 정상적인 속도는 분당 120~180 단어 사이입니다.
...
이것은 정보 흐름의 문제가 실제로 한 사람에게 맞닥뜨리는 곳입니다.
...
이 글의 주제는 첨단 기술이 중증 장애인의 의사소통과 일상생활을 어떻게 변화시키고, 그 삶의 가능성을 크게 확장시켰는가 입니다.

- 스티븐 호킹 교수는 기술이 장애인의 삶과 소통 방식을 혁신적으로 변화시켰다고 강조함.  
- 장애인의 가장 큰 의사소통 문제는 ‘보(baud) 속도’—즉, 정보를 전달할 수 있는 속도가 매우 낮다는 점이라고 설명함.  
- 일반적인 말하기 속도는 매우 높은 정보량을 전달하지만, 장애인의 경우 스위치를 이용한 입력은 초당 2~3비트 수준으로 크게 제한됨.  
- 이를 보완하기 위해 전체 문장을 글자별로 입력하는 대신, 단어·구 목록에서 선택하는 방식이 훨씬 효율적이라고 말함.  
- 마이크로프로세서 발전 덕분에 저렴한 비용으로 높은 성능의 컴퓨팅이 가능해졌고, 이를 통해 스위치 입력을 문장으로 변환하는 소프트웨어(예: Equalizer)가 사용 가능해짐.  
- 본인은 Equalizer 프로그램을 통해 분당 약 15단어를 말할 수 있으며, 이는 개선 여지가 있지만 충분히 실용적이라고 언급함.  
- 음성 합성 기술 역시 발전하여, 과거의 부자연스러운 음성 대신 거의 인간적인 음성이 가능해졌다고 설명함.  
- 또한 적외선·라디오 기반 환경 제어, 휴대전화 등 다양한 기술이 장애인의 일상생활을 지원함을 언급함.  
- “만약 장애를 겪는다면, 지금 시대에 태어난 것이 그나마 행운”이라며, 기술 덕분에 생존과 저술 활동이 가능했다고 밝힘.  
- 마지막으로 기술이 장애인에게 가져다준 혜택에 감사하며 연설을 마무리함.