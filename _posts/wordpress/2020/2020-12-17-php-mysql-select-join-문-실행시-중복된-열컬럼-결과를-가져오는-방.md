...
PHP에서 결과를 얻을 때 컬럼 이름을 `$row['컬럼이름']`으로 가져오는 경우, 동일한 컬럼 이름을 가진 뉴스 ID(`id`)와 사용자 ID(`id`)를 얻으려면 어떻게해야 합니까? 
...
### **(2) 두 번째 방법**
`*` 을 통해 컬럼 전부를 가져온 뒤, 필요한 컬럼만 뒤에 별칭을 기입해 지정 (추천)
