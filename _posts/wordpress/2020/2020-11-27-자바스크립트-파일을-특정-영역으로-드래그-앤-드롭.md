...
아래 화면처럼 브라우저의 외부 파일 탐색기에서 파일을 드래그 앤 드랍하여 `<input type=file>`에 파일을 입력받는 방법입니다. 호환성을 위해 ES5 이하 스펙으로 작성하였습니다.
...
{% gist "656003f348da640679cea7e0badc0998" %}
...
- 브라우저 기본 동작 방지와 버블링 방지 목적으로 `e.preventDefault()`, `e.stopPropagation()`을 추가합니다. ([관련 글](/posts/자바스크립트-html-이벤트-버블링-캡처링-위임delegation-동적/)
