...
`NamedShape`클래스의 또 다른 하위 클래스인 `Circle`을 작성하시오. 이 클래스는 이니셜라이저를 통해 `radius`와 `name`을 인자로 받는다. Circle 클래스 안에 `area`, `simpleDescription` 함수를 구현하시오.
...
optionalName에 nil을 할당하고, 할당된 값이 nil일 때 "Hell, Anonymous"을 greeting에 할당하도록 else 절을 추가하시오.
