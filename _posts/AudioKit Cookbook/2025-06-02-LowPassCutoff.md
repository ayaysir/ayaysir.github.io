...
**저주파수(낮은 소리)는 통과시키고, 고주파수(높은 소리)는 차단**하는 필터입니다.
...
"어디까지를 **통과**시키고, 그 이상부터는 **감쇄**시킬지"를 결정하는 경계점입니다
...
AudioKit의 `Delay` 노드는 내부적으로 딜레이된 신호가 **점점 줄어들며 반복**되는데, 여기서 **고주파 성분이 계속 남아 있으면 날카로운 잔향**이 생깁니다.
...
필요에 따라 `cutoff frequency`를 조절하여, **딜레이 톤을 "밝게" 혹은 "어둡게" 만드는 도구**로 생각하면 됩니다.
